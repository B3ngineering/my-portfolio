    ...
    <div className="bg-black min-h-screen">
          ...
          <div className="lg:grid lg:grid-cols-2 lg:gap-8 items-center">
            <div className="text-center lg:text-left">
              ...
              <div className="mt-8 flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
                ...
                >
                  <span className="mr-2">&gt;</span> VIEW_WORK.EXE
                </Link>
                <Link
                  href="/about"
                ...
                >
                  <span className="mr-2">&gt;</span> ABOUT_ME.TXT
                </Link>
              </div>
            </div>
            ...
            <div className="bg-black border-2 border-matrix-400 p-6 shadow-terminal hover:shadow-matrix transition-all duration-300">
              ...
              <div className="flex flex-wrap gap-2">
                {skills.backend.map((skill: string, index: number) => (
                  <span
                    key={index}
                    className="inline-block bg-terminal-100 border border-green-300 text-green-300 text-sm px-3 py-1 font-jetbrains-mono hover:bg-green-300 hover:text-black transition-colors duration-200"
                  >
                    {skill}
                  </span>
                ))}
            ...
            <div className="bg-black border-2 border-matrix-400 p-6 shadow-terminal hover:shadow-matrix transition-all duration-300">
              ...
              <div className="flex flex-wrap gap-2">
                {[...skills.database, ...skills.tools].map((skill: string, index: number) => (
                  <span
                    key={index}
                    className="inline-block bg-terminal-100 border border-green-200 text-green-200 text-sm px-3 py-1 font-jetbrains-mono hover:bg-green-200 hover:text-black transition-colors duration-200"
                  >
                    {skill}
                  </span>
                ))}
            ...
            <div className="bg-black border-2 border-matrix-400 p-6 shadow-terminal hover:shadow-matrix transition-all duration-300">
              ...
              <div className="flex flex-wrap gap-2">
                {skills.mobile.map((skill: string, index: number) => (
                  <span
                    key={index}
                    className="inline-block bg-terminal-100 border border-matrix-400 text-matrix-400 text-sm px-3 py-1 font-jetbrains-mono hover:bg-matrix-400 hover:text-black transition-colors duration-200"
                  >
                    {skill}
                  </span>
                ))}
      ...
      <section className="py-16 sm:py-20 bg-black border-t-2 border-matrix-400">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-lg text-green-200 mb-8 max-w-2xl mx-auto font-jetbrains-mono border-l-2 border-matrix-400 pl-4">
            <span className="text-matrix-400">&gt;&gt;&gt;</span> Always ready to decrypt new opportunities and compile innovative solutions.
          </p>
